...
def getUserEmail(id):
    ...
    try:
        user = user_models.UserProfile.objects.get(id=id)
        return user.email
    except:
        return None
...
def getTentID(userId):
    ...
    try:
        tents = tent_models.TentGroup.objects.get(Q(tenter_1=userId) | Q(tenter_2=userId) | Q(tenter_3=userId) |
...
def getTenterInformation(tentId):
    try:
        tent = tent_models.TentGroup.objects.get(id=tentId)
        users = {
